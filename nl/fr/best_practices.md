...
- Pour maintenir une connexion permanente entre les composants de votre réseau, utilisez des [signaux de présence gRPC](https://github.com/grpc/grpc/blob/master/doc/keepalive.md){: external}. les signaux de présence maintiennent la connexion gRPC active et empêchent la fermeture d'une connexion "inutilisée". L'exemple suivant d'une connexion d'homologue ajoute des options gRPC à l'objet [Options de connexion](https://fabric-sdk-node.github.io/global.html#ConnectionOpts){: external}. Les options gRPC sont définies sur des valeurs recommandées par {{site.data.keyword.blockchainfull_notm}} Platform.  
  ```
  var peer = fabric_client.newPeer(creds.peers["org1-peer1"].url, { pem: creds.peers["org1-peer1"].tlsCACerts.pem , 'ssl-target-name-override': null},
  "grpcOptions": {
...
Si vous utilisez le logiciel SDK Node, vous pouvez définir les valeurs de délai d'expiration directement dans la méthode appelée. Par exemple, vous pouvez utiliser la ligne ci-dessous pour augmenter la valeur du délai pour [instancier un code blockchain](https://fabric-sdk-node.github.io/Channel.html#sendInstantiateProposal){: external} sur 5 minutes.
```
...
Pour plus d'informations sur CouchDB et la façon de définir des index, voir [CouchDB as the State Database](https://hyperledger-fabric.readthedocs.io/en/release-1.4/couchdb_as_state_database.html){: external} dans la documentation Hyperledger Fabric. Vous trouverez également un exemple utilisant un index avec un code blockchain dans le [tutoriel Fabric CouchDB](https://hyperledger-fabric.readthedocs.io/en/release-1.4/couchdb_tutorial.html){: external}.
...
Des exemples illustrant comment les requêtes utilisent les index et le type de requêtes présentant les meilleures performances sont fournis dans [tutoriel Fabric CouchDB](https://hyperledger-fabric.readthedocs.io/en/release-1.4/couchdb_tutorial.html#use-best-practices-for-queries-and-indexes){: external}.
