...
Si vous êtes un utilisateur doté du rôle de gestionnaire, vous pouvez retirer l'accès d'un utilisateur à la console. Sous l'onglet **Utilisateurs** de la console, cliquez sur les trois points verticaux en fin de ligne d'un utilisateur donné, puis cliquez sur **Supprimer un utilisateur authentifié**. L'utilisateur ne peut plus se connecter à la console.
...
| **Demande** |  |
|-------------|-----------|
| Path | PUT `<API_endpoint>`/ak/api/v1/permissions/keys |
| **Zones du corps de la demande** | |
| <ul><li>`users`</li><li>`roles`</li></ul> | <ul><li>`string` user id </li><li>`["reader", "writer", "manager"]` Au moins un valeur est obligatoire </li></ul> |
| **Zones du corps de la réponse** | |
| <ul><li>`uuids`</li></ul>| <ul><li>`string` user id</li></ul>|
...
| **Demande** |  |
|-------------|-----------|
| Path | DELETE `<API_endpoint>`/ak/api/v1/permissions/keys |
| **Zones du corps de la demande** | |
...
La console expose les mêmes API que celles utilisées par {{site.data.keyword.blockchainfull_notm}} Platform for {{site.data.keyword.cloud_notm}}. Vous pouvez consulter la [Référence d'API d'{{site.data.keyword.blockchainfull_notm}} Platform](https://test.cloud.ibm.com/apidocs/blockchain) pour voir la liste complète des API. Comme vous utilisez des API pour communiquer avec votre console sur {{site.data.keyword.cloud_notm}} Private, vous devez utiliser l'autorisation fournie par {{site.data.keyword.cloud_notm}} avec l'authentification fournie par votre console. Remplacez `Bearer Auth` dans la référence d'API par `-u <api_key>:<api_secret>`dans la commande curl. Vous ne pouvez pas utiliser l'onglet **Essayez** pour tester les API des réseaux sur {site.data.keyword.cloud_notm}} Private.

Vous pouvez utiliser des API pour créer des noeuds dans le cluster où votre console est déployée, et importer des noeuds d'autres clusters ou {{site.data.keyword.cloud_notm}}. Pour plus d'informations, voir [Génération d'un réseau à l'aide d'API](/docs/services/blockchain/howto/ibp-v2-apis.html#ibp-v2-apis-build-with-apis) et [Importation d'un réseau à l'aide d'API](/docs/services/blockchain/howto/ibp-v2-apis.html#ibp-v2-apis-import-with-apis).
