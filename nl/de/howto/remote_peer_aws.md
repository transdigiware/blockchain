...
Weitere Informationen zu AWS finden Sie im Dokument mit der [AWS-Übersicht](https://d1.awsstatic.com/whitepapers/aws-overview.pdf){: external}.
...
1. Wählen Sie eine der folgenden Optionen aus, um die AWS CloudFormation-Vorlage in Ihrem AWS-Konto zu starten. Hilfe bei der Auswahl einer Option bieten Ihnen die Abschnitte, in denen die Bereitstellungsoptionen bereits in diesem Handbuch beschrieben worden sind. Für jede Bereitstellung benötigen Sie ungefähr 10 Minuten.  
  ...
  * [{{site.data.keyword.blockchainfull_notm}} Platform for AWS in einer neuen VPC unter AWS bereitstellen](https://fwd.aws/v43nk){: external}.  

  * [{{site.data.keyword.blockchainfull_notm}} Platform for AWS in vorhandener VPC unter AWS bereitstellen](https://fwd.aws/zrP4g){: external}.
...
In den folgenden Tabellen sind die Parameter nach Kategorien sortiert aufgelistet und für die beiden Bereitstellungsoptionen separat beschrieben:
  ...
  * [Parameter für die Bereitstellung von {{site.data.keyword.blockchainfull_notm}} Platform for AWS in neuer VPC](/docs/services/blockchain/howto?topic=blockchain-remote-peer-aws#remote-peer-aws-parameters-newvpc)

  * [Parameter für die Bereitstellung von {{site.data.keyword.blockchainfull_notm}} Platform for AWS in vorhandener VPC](/docs/services/blockchain/howto?topic=blockchain-remote-peer-aws#remote-peer-aws-parameters-existvpc)
...
| `Zulässiger CIDR für SSH-Zugriff` | Zulässiger [CIDR-Block](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Subnets.html#vpc-resize){: external} für externen SSH-Zugriff auf die {{site.data.keyword.blockchainfull_notm}}-Peerinstanzen. Als Wert kann 0.0.0.0/0 angegeben werden, um den Zugriff von jedem beliebigen Standort aus zuzulassen (dies wird allerdings nicht empfohlen). | |
| `CIDR für Peerendpungzugriff` | Zulässiger [CIDR](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Subnets.html#vpc-resize){: external}-Block für externen gRPC-Zugriff auf die {{site.data.keyword.blockchainfull_notm}}-Peerinstanzen. Als Wert ist normalerweise 0.0.0.0/0 angegeben, um den Zugriff von jedem beliebigen Standort aus zuzulassen (dies wird allerdings nicht empfohlen). | |
| | | |
...
| | | |
|**{{site.data.keyword.blockchainfull_notm}}-Serviceberechtigungsnachweise**| | |
...
Falls Sie den Peer von {{site.data.keyword.blockchainfull_notm}} Platform for AWS in einer vorhandenen VPC bereitstellen, sollten Sie die folgenden Hinweise beachten:
 ...
 - Konfigurieren Sie die Option für den Domänennamen in den DHCP-Optionen gemäß der Erläuterung in der [Amazon-Dokumentation für die VPC](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_DHCP_Options.html){: external}.  
...
- Erstellen Sie Sicherheitsgruppe, die an Ihre vorhandene VPC gebunden ist, und fügen Sie Regeln für eingehende Daten an den Ports 22 und 7051 zu dieser Sicherheitsgruppe hinzu. TCP-Verbindungen an Port 22 lassen den SSH-Zugriff auf die generierte Instanz zu, während TCP-Verbindungen an Port 7051 einen externen gRPC-Zugriff auf die Peerinstanz ermöglichen (erforderlich für den Betrieb des Peers mithilfe der CLI für die Fabric-Tools und der Fabric-SDKs). Diese VCP-Einstellungen werden von Ihnen angefordert, wenn Sie den Schnelleinstieg starten.
 ...
 Bei der Bereitstellung eines Peers von {{site.data.keyword.blockchainfull_notm}} Platform for AWS in einem vorhandenen VPC ersetzen die folgenden Parameter die Parameter in den entsprechenden [obigen](/docs/services/blockchain/howto?topic=blockchain-remote-peer-aws#remote-peer-aws-parameters-newvpc) Abschnitten:
...
{{site.data.keyword.blockchainfull_notm}} Platform Enterprise Plan verwendet die Verschlüsselung ganzer Platten, die auf der [Verschlüsselung mit symmetrischen Schlüsseln](https://www.ibm.com/support/knowledgecenter/en/SSB23S_1.1.0.14/gtps7/s7symm.html){: external} basiert, um alle im Netz verwendeten Daten zu schützen. In Ihrer eigenen Umgebung müssen Sie zum Schutz Ihrer Peerdaten ähnliche Schritte unternehmen.
