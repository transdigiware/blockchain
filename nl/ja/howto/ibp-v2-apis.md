...
このチュートリアルでは、{{site.data.keyword.blockchainfull_notm}} Platform API を使用してブロックチェーン・ネットワークを構築する一般的な流れについて説明します。 各 API について詳しくは、[{{site.data.keyword.blockchainfull_notm}} Platform API 参照資料](/apidocs/blockchain){: external}を参照してください。

これらの API は、{{site.data.keyword.blockchainfull_notm}} Platform for {{site.data.keyword.cloud_notm}} v0.1.77 以降のみと互換性があります。 バージョンを確認するには、`https://[your_console_url]/version.txt` を参照します。ここで *`[your_console_url]`* は {{site.data.keyword.blockchainfull_notm}} Platform コンソールの URL です。 例えば、https://1ee1869ffa6496d6bc1ce4b-optools.bp01.blockchain.cloud.ibm.com/version.txt などです。
{:note}
...
1. [{{site.data.keyword.cloud_notm}} リソース・リスト](https://cloud.ibm.com/resources){: external}で、作成したブロックチェーン・サービス・インスタンスを開きます。
...
各 API の curl コマンドの例については、[{{site.data.keyword.blockchainfull_notm}} Platform API 参照資料](/apidocs/blockchain){: external}を参照してください。
...
API を使用して、API または {{site.data.keyword.blockchainfull_notm}} Platform コンソールを使用して作成された {{site.data.keyword.blockchainfull_notm}} コンポーネントを、{{site.data.keyword.blockchainfull_notm}} Platform の別のサービス・インスタンスにインポートすることもできます。
...
1. [Fabric CA クライアント](https://hyperledger-fabric-ca.readthedocs.io/en/release-1.4/users-guide.html#fabric-ca-client){: external}をローカル・ファイル・システムにダウンロードします。

  Fabric CA クライアントを入手する最も簡単な方法は、すべての Fabric ツール・バイナリーを直接ダウンロードすることです。 コマンド・ラインを使用してバイナリーをダウンロードするディレクトリーにナビゲートし、以下の [Curl](https://hyperledger-fabric.readthedocs.io/en/release-1.4/prereqs.html#install-curl){: external} コマンドを実行して、バイナリーを取り出します。
