...
スターター・プラン・ネットワークのログは、[{{site.data.keyword.cloud_notm}} Log Analysis サービス](https://cloud.ibm.com/catalog/services/log-analysis){: external}によって収集されます。 デフォルトでは、ログは Log Analysis サービスのライト・プランで収集されます。 このプランは無料であり、**ログは 3 日間保管**されてから廃棄されます。 また、**1 日当たりログの最初の 500 MB のみ検索可能**です。 ネットワーク・ログが 500 MB を超えると、Kibana で新規ログを表示することはできません。 ネットワークで 500 MB より大きいログが生成される場合や、3 日を超えてログを保持する場合は、有料版の Log Analysis サービスにアップグレードすることができます。
