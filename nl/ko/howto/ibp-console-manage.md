...
피어, 순서 지정자 및 인증 기관의 로그는 {{site.data.keyword.IBM_notm}} Kubernetes Service에 의해 수집됩니다. {{site.data.keyword.blockchainfull_notm}} Platform 네트워크를 배치한 클러스터의 노드 로그를 보려면 다음 단계를 수행하십시오. 

노드 로그를 더 쉽게 찾으려면 노드를 배치할 때 사용된 네임스페이스를 필터링하는 것이 유용합니다.
네임스페이스를 찾으려면 콘솔에서 원하는 CA 노드를 열고 **설정** 아이콘을 클릭하십시오. **인증 기관 엔드포인트 URL**의 값을 확인하십시오. 예를 들면 `https://n2734d0-paorg10524.ibpv2-cluster.us-south.containers.appdomain.cloud:7054`와 같습니다. 
...
기본적으로 노드의 로그는 클러스터 내에서 로컬로 수집됩니다. 또한 {{site.data.keyword.cloud_notm}} 서비스 또는 서드파티 서비스를 사용하여 네트워크에서 로그를 수집, 저장 및 분석할 수 있습니다. 자세한 정보는 [{{site.data.keyword.IBM_notm}} Kubernetes Service에 대한 로깅 및 모니터링](https://cloud.ibm.com/docs/containers?topic=containers-health#health){: external}을 참조하십시오. 로그를 실시간으로 쉽게 구문 분석할 수 있는 [{{site.data.keyword.cloud_notm}} LogDNA](https://cloud.ibm.com/docs/services/Log-Analysis-with-LogDNA?topic=LogDNA-kube#kube){: external} 서비스를 활용하는 것이 좋습니다. 
...
시간 경과에 따라 피어, CA 및 순서 지정자 노드에 대한 기본 {{site.data.keyword.IBM_notm}} Hyperledger Fabric Docker 이미지를 업데이트해야 할 수 있습니다(예: 보안 업데이트 사용 또는 새 Fabric 지점 릴리스로). 노드 타일의 **패치 사용 가능** 텍스트는 해당 패치가 사용 가능하고 준비가 될 때마다 노드에 설치될 수 있다는 표시기입니다. 이러한 패치는 선택사항이지만 권장됩니다.  콘솔로 가져온 노드는 패치할 수 없습니다. 
...
노드에 패치를 적용하려면 노드 타일을 열고 **패치 설치** 단추를 클릭하십시오.
...
이 조치가 완료되면 [원래의 단계](/docs/services/blockchain/howto/ibp-v2-deploy-iks.html#ibp-v2-deploy-iks)를 수행하여 {{site.data.keyword.cloud_notm}}  카탈로그 페이지에서 Kubernetes 클러스터 및 블록체인 서비스 인스턴스를 새로 작성할 수 있습니다. 
