...
네트워크 모니터의 "개요" 화면에는 인증 기관, 순서 지정 서비스 및 피어 상태가 표시됩니다. **조치** 헤더의 드롭다운 목록에서 **로그 보기**를 클릭하여 특정 네트워크 컴포넌트의 로그를 보십시오. 엔터프라이즈 플랜 네트워크를 사용하는 경우 텍스트 파일 형식으로 컴포넌트 로그를 볼 수 있습니다. 스타터 플랜 네트워크를 사용하는 경우 컴포넌트 로그는 [{{site.data.keyword.cloud_notm}} 로그 분석 서비스](https://cloud.ibm.com/catalog/services/log-analysis){: external}를 통해 수집되며, 사용자는 [Kibana](/docs/services/blockchain/howto/monitor_network.html#monitor-blockchain-network-viewing-kibana-logs)로 로그를 볼 수 있습니다. 
...
Hyperledger Fabric은 메시지 심각도에 따라 여러 [로깅 레벨](https://hyperledger-fabric.readthedocs.io/en/release-1.2/logging-control.html){: external}을 제공합니다. {{site.data.keyword.blockchainfull_notm}} Platform의 기본 로깅 레벨은 `INFO`입니다. 추가 로그를 보려면 [지원 티켓](/docs/services/blockchain/ibmblockchain_support.html#blockchain-support-cases)을 열어 로깅 레벨을 더 자세한 `DEBUG`로 설정할 수 있습니다. `DEBUG` 레벨 로그에서는 필터링해야 할 수도 있는 다수의 사소한 메시지를 표시한다는 점에 유의하십시오. 메시지에서 `warning` 또는 `error`를 검색하여 Hyperledger Fabric 컴포넌트의 문제점을 발견하십시오. 컴포넌트 컨테이너가 실패하는지 아니면 강제 종료되는지 발견하려면 {{site.data.keyword.cloud_notm}}에서 보낸 `panic` 또는 `killed` 메시지를 검색하십시오.
...
오른쪽 상단 모서리에 있는 시간 범위 단추를 사용하여 로그가 표시되는 기간을 변경할 수 있습니다. 화면 왼쪽의 탭을 사용하여 검색에서 필드를 제거하고 추가할 수 있습니다. 표시할 가장 중요한 필드는 메시지 필드입니다. 시간소인 없이 메시지를 검색하면 해당 메시지 로그의 모든 인스턴스를 찾는 데 도움이 될 수 있습니다. **저장** 단추를 눌러 현재 검색을 저장하고 특정 보기로 돌아가십시오. 데이터를 Kibana로 표시하는 작업에 대한 자세한 정보는 [Kibana 사용자 안내서](https://www.elastic.co/guide/en/kibana/6.2/index.html){: external}를 참조하십시오. 로그 분석 CLI를 사용하여 로컬 파일 시스템에 [로그를 다운로드](https://cloud.ibm.com/docs/services/CloudLogAnalysis/how-to/manage-logs/downloading_logs_cloud.html#downloading_logs){: external}할 수도 있습니다.
