...
If none of the ordering nodes that were consenters on the channel are still up (as is the case at the end of the migration process), any peer attempting to join the channel will try and fail to connect to the nodes listed in the genesis block and be unable to proceed past that point.
