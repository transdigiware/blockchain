...
In addition to being able to link your {{site.data.keyword.blockchainfull}} Platform instance to an {{site.data.keyword.cloud_notm}} Kubernetes service cluster, you can now also link your platform service instance to a Red Hat OpenShift cluster that is running in {{site.data.keyword.cloud_notm}}. The process is identical, the only difference is that when you link a cluster, you can now select your OpenShift cluster from the list of available clusters. See [Getting started with {{site.data.keyword.blockchainfull}} Platform for {{site.data.keyword.cloud_notm}}](/docs/blockchain?topic=blockchain-ibp-v2-deploy-iks) to review the deployment process.
