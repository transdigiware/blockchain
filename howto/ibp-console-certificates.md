...
By default, certificates that are generated by {{site.data.keyword.blockchainfull_notm}} Platform CAs expire after one year. TLS certificates, on the other hand, expire after:
...
If the CA is offline, unreachable, or the renewal fails, the certificates cannot be automatically renewed. The peer or ordering node displays a Warning message indicating that certificate expiration is approaching and you need to manually update it. In that case, you can follow the instructions in the [Manual certificate renewal](#cert-mgmt-manual-renewal) to update these certificates.
...
Organization MSP admin certificates cannot be automatically renewed. Instead, you need to follow a [manual process](#cert-mgmt-manual-renewal) to update these certificates. If you have a large number of channels on your network, an [Ansible playbook](#cert-mgmt-bulk-ansible) is available to update certificates across your network in a bulk action.
