...
The left column includes the certificates for the blockchain CA, peer and ordering nodes. There are two certificates included on the CA node, the CA root cert and the CA TLS cert. The root certificate is included here for completeness, but because it expires in 15 years, update instructions are not provided. The peer and ordering nodes are similar in that they both contain an **enrollment certificate** (the public key or sign cert), their **TLS certificate** that enables them to transact with other nodes in their organization, and their **MSP admin certificate**, which represents the admin identity allowed to administer the node. The peer admin can install smart contracts however, the ordering node admin has no special privileges.   

The {{site.data.keyword.blockchainfull}} Platform  can automatically renew the enrollment certs for the peer and ordering nodes and the TLS cert for the peer. But the MSP admin identities have to be manually renewed. If the MSP is enabled for Node OU support, no further action is required. More information about Node OU support and how to determine if the MSP is enabled for it is provided in this topic.

The right column shows the certificates that are relevant to the system and application channel which is based on the system channel. When the MSP is not enabled for Node OU support, additional steps are required to update these certificates on the system and application channels and are provided here.

## Certificate renewal and expiration
{: #cert-mgmt-renew-expiration}

By default, all certificates that are generated by {{site.data.keyword.blockchainfull_notm}} Platform CAs expire after one year, except for the CA root cert that expires after 15 years. And TLS certificates, expire after:
...
As described in Figure 1, the console attempts to automatically renew enrollment certificates for peers and ordering nodes and the peer TLS certificate. However, admin certificates need to be updated manually.
...
If the updated certificate if is an admin certificate for a peer or ordering service, you need to update the node admin identity. Open the associated peer or ordering service and click the **Associate identity for peer** or **Associate identity for ordering service** and browse to the newly created identity from the wallet.
