...
- Because creating a CA replica set results in a new CA pod, when you configure an additional replica set, the CPU and memory requirements for CA are doubled. If three replica sets are required, then the total CPU and memory requirements triple, and so on. However, the storage allocation does not change because all of the replica sets use the same database. When you configure your CA, you can specify how many replica sets to create.
...
Use of a third-party PostgreSQL database is also supported.  If the third-party database is configured to use TLS, you will need to provide at least one server certificate. If SSL client authentication (mutual TLS) is enabled, you will also need to provide a client certificate and a client key in .pem format.  

##  Creating the PostgreSQL service credentials file
...
If you are not using TLS, you should remove the `certificate` and `client` elements from your JSON file.  
{: important}
...
2. Make sure the option to **Create** a Certificate Authority is selected. Then click **Next**.
