...
* **Ordering service consenters**. The ordering service nodes on a particular channel are known in a Raft consensus mechanism as a "consenter set". It is possible to add or remove particular nodes from this consenter set after a channel has been created. For information about updating the consenter set, see the [Adding and removing ordering service consenters](/docs/services/blockchain/howto?topic=blockchain-ibp-console-govern#ibp-console-govern-consenters) section below.
